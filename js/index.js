document.addEventListener('DOMContentLoaded', () => {
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', (e) => {
      e.preventDefault();
      alert('Thank you for reaching out! I will be in touch with you shortly to set up your custom review flow.');
      contactForm.reset();
    });
  }
  ...
  function renderCustomers() {
    ...
    if (filtered.length === 0) {
      customersGrid.innerHTML = '<div style="grid-column: 1/-1; text-align: center; color: var(--text-muted); padding: 40px 0;">No customers found matching this filter.</div>';
      return;
    }
    ...
    filtered.forEach(customer => {
      ...
      const dateFormatted = new Date(customer.launchDate).toLocaleDateString('en-US', dateOptions);

      // Create rating stars representation
      const fullStars = Math.floor(customer.rating);
      const halfStar = customer.rating % 1 !== 0;
      const starsStr = '★'.repeat(fullStars) + (halfStar ? '½' : '') + '☆'.repeat(5 - Math.ceil(customer.rating));
